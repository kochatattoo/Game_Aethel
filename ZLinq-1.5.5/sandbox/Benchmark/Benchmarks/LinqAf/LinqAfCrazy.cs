...
namespace Benchmark.LinqAf
{
    public class Crazy
    {
        ...
        public class Giant
        {
            ...
            {
                var giant =
                    ValueEnumerable.Range(0, 1000)
                        .Where(x => x % 2 == 0)
                        .Select(x => new[] { x, x * x })
                        .Concat(
                            ValueEnumerable.Repeat(new int[] { 3, 4, 5 }, 6)
                        )
                        .SelectMany(y => y.AsValueEnumerable())
                        .Union(
                            ValueEnumerable.Empty<int>().DefaultIfEmpty(999)
                        )
                        .Intersect(
                            ValueEnumerable.Range(500, 10000)
                        )
                        .Except(new[] { 1001, 1002, 1003, 2001 }.AsValueEnumerable())
                        .Join(ValueEnumerable.Range(1, 10000), x => x, y => y, (a, b) => (a + b) / 2)
                        .GroupBy(x => x % 5)
                        .Select(x => new[] { x.AsValueEnumerable().First(), x.AsValueEnumerable().FirstOrDefault(), x.AsValueEnumerable().Last(), x.AsValueEnumerable().LastOrDefault() })
                        .SelectMany(x => x)
                        .GroupJoin(ValueEnumerable.Range(1, 10000), x => x, y => y, (a, b) => b.AsValueEnumerable().FirstOrDefault(z => true))
                        .Take(90000)
                        .Skip(1)
                        .TakeWhile(x => true)
                        .SkipWhile(x => false)
                        ...
                        .OrderBy(x => x)
                        .ThenBy(y => y)
                        .Zip(ValueEnumerable.Range(1, 100), (a, b) => a + b)
                        .Distinct();
                ...
                // ZLinq can't use AsEnumerable so use raw loop instead
                using (var e = giant.Enumerator)
                {
                    while (e.TryGetNext(out var i))
                    {
                        GC.KeepAlive(i);
                    }
                }
