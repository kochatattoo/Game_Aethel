...
namespace ZLinq.Tests
{
    public class ConcatTests : EnumerableTests
    {
        ...
        {
            AssertExtensions.Throws<ArgumentNullException>(() => ((IEnumerable<int>)null).Concat(Enumerable.Range(0, 0)));
            AssertExtensions.Throws<ArgumentNullException>(() => ((IEnumerable<int>)null).Concat(null)); // If both inputs are null, throw for "first" first
        }
