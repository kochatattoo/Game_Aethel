...
namespace ZLinq.Tests
{
    public class ToDictionaryTests : EnumerableTests
    {
        ...
        {
            validation(items.ToDictionary(key => key));
            validation(items.ToDictionary(key => key, value => value));
            validation(items.ToDictionary(key => key));
            validation(items.ToDictionary(key => key, value => value));
        ...
        private void RunToDictionaryFromKvOnAllCollectionTypes<T>(T[] items, Action<Dictionary<T, T>> validation) where T : notnull
        {
            ...
            validation(kvps.ToDictionary(x => x.Key, x => x.Value));
            ...
            validation(new List<(T, T)>(vts).ToDictionary(x => x.Item1, x => x.Item2));
            validation(new TestEnumerable<(T, T)>(vts).ToDictionary(x => x.Item1, x => x.Item2));
            validation(new TestReadOnlyCollection<(T, T)>(vts).ToDictionary(x => x.Item1, x => x.Item2));
            validation(new TestCollection<(T, T)>(vts).ToDictionary(x => x.Item1, x => x.Item2));
        }
