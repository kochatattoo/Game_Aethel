...
namespace ZLinq.Tests
{
    public class ShuffleTests : EnumerableTests
    {
        ...
        public void ForcedToEnumeratorDoesntEnumerate()
        {
            ...
            var valueEnumerable1 = NumberRangeGuaranteedNotCollectionType(0, 3).Shuffle();
            var en1 = valueEnumerable1.Enumerator;
            Assert.False(en1.TryGetNext(out _));

            var valueEnumerable2 = NumberRangeGuaranteedNotCollectionType(0, 3).Shuffle().Take(1);
            var en2 = valueEnumerable2.Enumerator;
            Assert.False(en2.TryGetNext(out _));

            var valueEnumerable3 = NumberRangeGuaranteedNotCollectionType(0, 3).Shuffle().Take(2).Take(1);
        ...
        public void ValidateShuffleTakeRandomDistribution(int mode)
        {
            ...
            {
                var selected = source.Shuffle().Take(1);
                ...
                for (int i = 0; i < Iterations; i++)
                {
                    ...
                    {
                        case 0:
                            using (var e = selected.GetEnumerator())
                            {
                                e.MoveNext();
                                value = e.Current;
                            }
