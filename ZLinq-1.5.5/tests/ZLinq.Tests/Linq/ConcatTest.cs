...
public class ConcatTest
{
    ...
    public void CountOfConcatIteratorShouldThrowExceptionOnIntegerOverflow()
    {
        ...
        Assert.Throws<OverflowException>(() => tinyCollection.AsValueEnumerable().Concat(tinyCollection).Concat(supposedlyLargeCollection).Count());
        Assert.Throws<OverflowException>(() => tinyCollection.AsValueEnumerable().Concat(tinyCollection).Concat(tinyCollection).Concat(supposedlyLargeCollection).Count());

        //// This applies to ToArray() and ToList() as well, which try to preallocate the exact size
        //// needed if all inputs are ICollections.
        Assert.Throws<OverflowException>(() => supposedlyLargeCollection.AsValueEnumerable().Concat(tinyCollection).ToArray());
        Assert.Throws<OverflowException>(() => tinyCollection.AsValueEnumerable().Concat(tinyCollection).Concat(supposedlyLargeCollection).ToArray());
        Assert.Throws<OverflowException>(() => tinyCollection.AsValueEnumerable().Concat(tinyCollection).Concat(tinyCollection).Concat(supposedlyLargeCollection).ToArray());

        Assert.Throws<OverflowException>(() => supposedlyLargeCollection.AsValueEnumerable().Concat(tinyCollection).ToList());
        Assert.Throws<OverflowException>(() => tinyCollection.AsValueEnumerable().Concat(tinyCollection).Concat(supposedlyLargeCollection).ToList());
        Assert.Throws<OverflowException>(() => tinyCollection.AsValueEnumerable().Concat(tinyCollection).Concat(tinyCollection).Concat(supposedlyLargeCollection).ToList());
    }
}
