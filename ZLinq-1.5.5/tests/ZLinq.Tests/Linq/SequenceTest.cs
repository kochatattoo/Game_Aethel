...
public class SequenceTests
{
    ...
    public void EndOutOfRange_Throws()
    {
        ...
        static void ValidateSigned<T>() where T : INumber<T>
        {
            ...
            for (int i = 1; i < 3; i++)
            {
                Assert.NotNull(ValueEnumerable.Sequence(T.CreateTruncating(123), T.CreateTruncating(122), T.CreateTruncating(-i)).ToArray());
            }
        ...
        static void ValidateUnsigned<T>() where T : INumber<T>
        {
            for (int i = 0; i < 3; i++)
            {
                Assert.NotNull(ValueEnumerable.Sequence(T.CreateTruncating(123), T.CreateTruncating(123), T.CreateTruncating(i)).ToArray());
            }

            for (int i = 1; i < 3; i++)
            {
                Assert.NotNull(ValueEnumerable.Sequence(T.CreateTruncating(123), T.CreateTruncating(124), T.CreateTruncating(i)).ToArray());
            }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>("endInclusive", () => ValueEnumerable.Sequence(start, endInclusive, step));
        }
    }
            ...
            for (int start = startStart; start <= startEnd; start++)
            {
                for (int count = 1; count <= 20; count++)
                {
                    Assert.Equal(
                        ...
                        ValueEnumerable.Sequence<T>(T.CreateTruncating(start), T.CreateTruncating(start + count - 1), T.One).ToArray());
                }
            }
        }
    }
    ...
    public void Numbers_ProduceExpectedSequence()
    {
        ...
        {
            Assert.Equal([T.Zero], ValueEnumerable.Sequence(T.Zero, T.Zero, T.One).ToArray());

            Assert.Equal([T.Zero, T.One, T.CreateTruncating(2), T.CreateTruncating(3), T.CreateTruncating(4)], ValueEnumerable.Sequence(T.Zero, T.CreateTruncating(4), T.One).ToArray());
            Assert.Equal([T.Zero, T.CreateTruncating(2), T.CreateTruncating(4)], ValueEnumerable.Sequence(T.Zero, T.CreateTruncating(4), T.CreateTruncating(2)).ToArray());
            Assert.Equal([T.Zero, T.CreateTruncating(3)], ValueEnumerable.Sequence(T.Zero, T.CreateTruncating(4), T.CreateTruncating(3)).ToArray());
            Assert.Equal([T.Zero, T.CreateTruncating(4)], ValueEnumerable.Sequence(T.Zero, T.CreateTruncating(4), T.CreateTruncating(4)).ToArray());
            Assert.Equal([T.Zero], ValueEnumerable.Sequence(T.Zero, T.CreateTruncating(4), T.CreateTruncating(5)).ToArray());

            Assert.Equal([T.CreateTruncating(42), T.CreateTruncating(45), T.CreateTruncating(48)], ValueEnumerable.Sequence(T.CreateTruncating(42), T.CreateTruncating(50), T.CreateTruncating(3)).ToArray());
            Assert.Equal([T.CreateTruncating(42), T.CreateTruncating(45), T.CreateTruncating(48), T.CreateTruncating(51)], ValueEnumerable.Sequence(T.CreateTruncating(42), T.CreateTruncating(51), T.CreateTruncating(3)).ToArray());
            Assert.Equal([T.MaxValue], ValueEnumerable.Sequence(T.MaxValue, T.MaxValue, T.One).ToArray());
            Assert.Equal([T.MaxValue], ValueEnumerable.Sequence(T.MaxValue, T.MaxValue, T.MaxValue).ToArray());
        }
        ...
        void ValidateSigned<T>() where T : INumber<T>, IMinMaxValue<T>
        {
            ...
            Assert.Equal([T.One, T.Zero, T.CreateTruncating(-1), T.CreateTruncating(-2), T.CreateTruncating(-3), T.CreateTruncating(-4)], ValueEnumerable.Sequence(T.One, T.CreateTruncating(-4), T.CreateTruncating(-1)).ToArray());
            Assert.Equal([T.One, T.CreateTruncating(-1), T.CreateTruncating(-3)], ValueEnumerable.Sequence(T.One, T.CreateTruncating(-4), T.CreateTruncating(-2)).ToArray());
            Assert.Equal([T.One, T.CreateTruncating(-2)], ValueEnumerable.Sequence(T.One, T.CreateTruncating(-4), T.CreateTruncating(-3)).ToArray());
            Assert.Equal([T.One, T.CreateTruncating(-3)], ValueEnumerable.Sequence(T.One, T.CreateTruncating(-4), T.CreateTruncating(-4)).ToArray());
            Assert.Equal([T.One, T.CreateTruncating(-4)], ValueEnumerable.Sequence(T.One, T.CreateTruncating(-4), T.CreateTruncating(-5)).ToArray());
            Assert.Equal([T.One], ValueEnumerable.Sequence(T.One, T.CreateTruncating(-4), T.CreateTruncating(-6)).ToArray());

            Assert.Equal([T.MinValue], ValueEnumerable.Sequence(T.MinValue, T.MinValue, T.CreateTruncating(-11)).ToArray());
        }
    }
